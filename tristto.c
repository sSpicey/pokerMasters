...
void atualizaTela()
{
   WriteConsoleOutputA(_tela_.wHnd, _tela_.buffer_tela, _tela_.characterBufferSize, _tela_.characterPosition, &(_tela_.consoleWriteArea));
}
...
void tprintf(int x, int y, COR corLetra, COR corFundo, const char* s, ...)
{
    va_list args;
    int i=0, tam;
    char buffer[ALTURA_TELA*LARGURA_TELA]; //e grande mesmo
    va_start(args, s);
...
int tscanf(int x, int y, COR corLetra, COR corFundo, const char* s, ...)
{
    ...
    do
    {
        ...
        {
            if(i>0)
                tputchar(' ', x+ --i, y, corLetra, corFundo);
        }
        else if(c!='\r' && c>31)
        {
...
char tputchar(unsigned char c, int x, int y, COR corLetra, COR corFundo)
{
    if(x<LARGURA_TELA && y<ALTURA_TELA && x>=0 && y>=0)
    {
        _tela_.buffer_tela[LARGURA_TELA*y + x].Attributes = (corLetra & 0x0F) | ((corFundo & 0x0F) << 4);
        _tela_.buffer_tela[LARGURA_TELA*(y++) + x].Char.AsciiChar = c;
        return c;
    }
    else
    {
...
void inicializaTela(const char s[])
{
    int x, y;
    HANDLE wHnd;
    HANDLE rHnd;
    ...
    //dependendo da versao do windows, a ordem muda. mais facil chamar duas vezes,
    //senao precisa sempre recompilar em cada SO
    SetConsoleWindowInfo(wHnd, TRUE, &windowSize);
...
{
    unsigned int c;
    const char* nomes[] = {"PRETO", "AZUL_ESCURO", "VERDE_ESCURO", "CIANO_ESCURO", "VERMELHO_ESCURO", "ROXO_ESCURO", "AMARELO_ESCURO", "CINZA_CLARO",
          "CINZA_ESCURO", "AZUL_CLARO", "VERDE_CLARO", "CIANO_CLARO", "VERMELHO_CLARO", "ROXO_CLARO", "AMARELO_CLARO", "BRANCO"};
    char str[] = {219,219,219,219,219,0};
    ...
    for(c = 0; c<256; c++)
    {
        tprintf(1+12*(c/32), 1+2*(c%32), BRANCO, PRETO, "%3d - %c    ", c, c);
    }

    //imprime tabela de cores
    for(c=0;c<16;c++)
    {
        tprintf(105, 20+2*c, BRANCO, PRETO, " : %s", nomes[c]);
        tprintf(100, 20+2*c, c, BRANCO, str);
    }

    tprintf(40, 66, AMARELO_CLARO, PRETO, "Pressione qualquer tecla para continuar...");
